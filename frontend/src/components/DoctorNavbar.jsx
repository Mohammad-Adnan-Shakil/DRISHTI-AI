 ...
 * - Online/Offline pill (navigator.onLine)
    ...
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md shadow-[0_4px_20px_rgba(0,0,0,0.03)] relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-3">
          ...
          {/* Right: Connectivity, Language, Doctor Profile & Logout */}
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#285943] to-[#16866A] text-white border-2 border-white shadow-sm flex items-center justify-center font-bold text-xs" title={`${doctorName} (${doctorRole})`}>
                {doctorInitials}
              </div>
              <div className="hidden lg:flex flex-col text-left">
                <span className="text-xs font-semibold text-[#20312A] leading-none">{doctorName}</span>
                <span className="text-[11px] text-[#66756D] leading-tight">{doctorRole}</span>
              </div>
            </div>
